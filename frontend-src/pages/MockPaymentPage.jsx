    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-slate-950 rounded-2xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-800">
        ...
        <div className="p-6">
          {status === 'idle' && (
            <>
              <div className="mb-8 text-center">
                <p className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Valor a pagar</p>
          ...
          {status === 'loading' && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-slate-600 dark:text-slate-400 font-bold animate-pulse">Processando pagamento...</p>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Pagamento Aprovado!</h2>
              <p className="text-slate-500 dark:text-slate-400">Você será redirecionado em instantes...</p>
            </div>
          )}
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 text-center border-t border-slate-200 dark:border-slate-800">
          <p className="text-xs text-slate-400 font-medium">Isso é apenas um teste. Nenhuma cobrança real será feita.</p>
        </div>
      </div>
    </div>
  );
