...
export default function InvoicesPage() {
      ...
      <div className="p-8 max-w-4xl mx-auto">
        <div className="bg-white border border-red-200 rounded-2xl p-8 text-center shadow-sm">
          ...
          <p className="text-slate-600 font-bold max-w-md mx-auto">
            ...
            permissões para acessar Notas Fiscais e Documentos de Entrada.
          </p>
        </div>
      </div>
    );
  ...
  const totalAmount = invoices.reduce(
    (sum, i) => (i.status !== 'CANCELED' ? sum + Number(i.totalAmount || 0) : sum),
    0,
  );
  const pendingLinkCount = invoices.filter(
    (i) => i.status === 'RECEIVED' || i.status === 'PENDING' || i.status === 'PENDING_REVIEW',
  ).length;
  const linkedCount = invoices.filter(
    (i) => i.status === 'LINKED' || i.status === 'COMPLETED',
  ).length;
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.items.length === 0 || !formData.items[0].ingredientId)
      return alert('Adicione pelo menos um insumo na nota.');
  ...
  const handleLinkPurchase = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/admin/invoices/${selectedInvoice.id}/link-purchase`,
        {
          method: 'PATCH',
          headers,
          body: JSON.stringify({
            ...
            notes:
              linkData.notes ||
              `Vinculado manualmente ao pedido #${linkData.purchaseOrderId?.slice(0, 8)}`,
          }),
        },
      );
      ...
      case 'COMPLETED':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
            <CheckCircle2 className="w-3.5 h-3.5" /> Conciliada / Vinculada
          </span>
        );
      case 'RECEIVED':
      case 'PENDING':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200">
            <Clock className="w-3.5 h-3.5" /> Recebida (Sem Vínculo)
          </span>
        );
      case 'PENDING_REVIEW':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200">
            <AlertCircle className="w-3.5 h-3.5" /> Em Revisão
          </span>
        );
      case 'CANCELED':
        return (
        ...
        {loading ? (
          ...
          <div className="p-16 text-center flex flex-col items-center justify-center">
            ...
            <p className="text-slate-600 font-semibold text-sm max-w-md mb-6">
              Nenhum documento fiscal de entrada corresponde à sua busca ou ainda não foram
              registrados.
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
                    ...
                    <tr key={inv.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="p-4">
                        <div className="font-bold text-slate-800">
                          {inv.supplier?.name || 'Fornecedor Desconhecido'}
                        </div>
                        {inv.supplier?.cnpj && (
                ...
                >
                  ...
                  {orders.map((o) => (
                    <option key={o.id} value={o.id}>
                      #{o.id.slice(0, 8)} — {o.supplier?.name} (R${' '}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-6">
            ...
            <div>
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3">
                Insumos da Nota (Entrada de Estoque)
              </h4>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-200 font-semibold">
              <div className="text-xs text-slate-600 font-bold">
                Fornecedor:{' '}
                <strong className="text-slate-950">{selectedInvoice.supplier?.name}</strong>
              </div>
              <div className="text-right">
                ...
                <span className="text-xl font-black text-emerald-700 font-mono">
                  {formatCurrencySafe(selectedInvoice.totalAmount)}
                </span>
              </div>
            </div>
          </div>
