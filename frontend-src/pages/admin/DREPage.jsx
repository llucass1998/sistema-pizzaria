...
import { BarChart3, RefreshCw, AlertCircle, ShieldAlert, CheckCircle2, HelpCircle, FileText, Download } from 'lucide-react';
    ...
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mt-1">DRE Simplificado</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-0.5 text-sm">Demonstração do Resultado do Exercício com apuração automática de margens e CMV.</p>
        </div>
      ...
      {isLoading && !dreData ? (
        <div className="flex flex-col items-center justify-center h-64 space-y-3">
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Calculando DRE e CMV dos itens...</p>
        </div>
      ) : dreData ? (
        <div className="space-y-6">
          ...
          {dreData.cmv?.status === 'PARTIAL' && (
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 flex items-start gap-3 print:hidden">
              ...
              <div className="text-xs md:text-sm text-amber-900 dark:text-amber-200">
                <span className="font-bold">Atenção ao CMV Estimado (Parcial):</span> Foram encontrados {dreData.cmv.unreliableProducts?.length || 0} produtos vendidos sem ficha técnica ou custos cadastrados. O sistema não inventou custos para esses itens a fim de preservar a precisão contábil.
              </div>
            </div>
          )}
          ...
          {dreData.cmv?.status === 'COMPLETE' && (
            <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50 rounded-xl p-4 flex items-center gap-3 print:hidden">
              ...
              <div className="text-xs md:text-sm text-emerald-900 dark:text-emerald-200 font-medium">
                <span className="font-bold">CMV 100% Preciso:</span> Todos os {dreData.cmv.itemsAnalyzed} itens vendidos no período possuem ficha técnica cadastrada no estoque.
              </div>
            </div>
          )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-6 md:p-8">
            <div className="border-b border-slate-200 dark:border-slate-800 pb-6 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Demonstração do Resultado do Exercício</h2>
                <p className="text-xs text-slate-500">Apuração baseada em receitas pagas e despesas liquidadas no período.</p>
              </div>
              <div className="text-right">
                <span className="text-xs font-bold text-slate-400 uppercase block">Margem Líquida</span>
                <span className={`text-2xl font-black ${dreData.netMargin >= 15 ? 'text-emerald-600 dark:text-emerald-400' : dreData.netMargin > 0 ? 'text-amber-500' : 'text-red-600'}`}>
            ...
            <div className="space-y-4 font-mono text-sm">
              ...
              <div className="flex items-center justify-between py-3 px-4 bg-slate-50 dark:bg-slate-950/40 rounded-xl">
                <div className="font-bold text-slate-800 dark:text-slate-200">
                  <span className="text-slate-400 mr-2">(+)</span> 1. Receita Bruta de Vendas ({dreData.breakdown?.paidOrdersCount || 0} pedidos pagos)
                </div>
              ...
              <div className="flex items-center justify-between py-2 px-4 text-red-600 dark:text-red-400 pl-8">
                <div>
                  <span className="mr-2">(-)</span> 2. Deduções e Pedidos Cancelados ({dreData.breakdown?.canceledOrdersCount || 0} cancelados)
                </div>
                <div className="font-bold">
                  ({formatCurrencySafe(dreData.deductions)})
                </div>
              </div>
              ...
              <div className="flex items-center justify-between py-2 px-4 text-amber-700 dark:text-amber-400 pl-8">
                <div>
                  <span className="mr-2">(-)</span> 4. Custo da Mercadoria Vendida (CMV Estimado via Fichas Técnicas)
              ...
              <div className="flex items-center justify-between py-2 px-4 text-red-600 dark:text-red-400 pl-8">
                <div>
                  <span className="mr-2">(-)</span> 6. Despesas Operacionais e Administrativas ({dreData.breakdown?.payablesCount || 0} contas pagas)
                </div>
                <div className="font-bold">
                  ({formatCurrencySafe(dreData.operatingExpenses)})
                </div>
              </div>
              ...
              <div className="flex items-center justify-between py-2 px-4 text-red-600 dark:text-red-400 pl-8">
                <div>
                  <span className="mr-2">(-)</span> 8. Impostos e Taxas de Gateway/Cartão (Estimativa 3.5%)
                </div>
                <div className="font-bold">
                  ({formatCurrencySafe(dreData.taxesAndFees)})
                </div>
              </div>

              {/* 9. Lucro Líquido Final */}
              <div className={`flex items-center justify-between py-4 px-6 rounded-2xl border-2 font-black text-lg ${dreData.netIncome >= 0 ? 'bg-emerald-500 text-white border-emerald-600 dark:bg-emerald-950/80 dark:text-emerald-300 dark:border-emerald-800' : 'bg-red-500 text-white border-red-600 dark:bg-red-950/80 dark:text-red-300 dark:border-red-800'}`}>
                <div>
                ...
                <div className="text-2xl">
                  {formatCurrencySafe(dreData.netIncome)}
                </div>
              </div>
            </div>
          </div>
          ...
          {dreData.cmv?.unreliableProducts?.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm print:hidden">
              ...
              <p className="text-xs text-slate-500 mb-4">
                Para que o CMV seja calculado com precisão centesimal, acesse o cardápio ou fichas técnicas e defina os ingredientes ou custo unitário dos seguintes itens:
              </p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                {dreData.cmv.unreliableProducts.map((p, idx) => (
                  <div key={idx} className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
                    <span className="font-bold text-slate-800 dark:text-slate-200 truncate">{p.name}</span>
                    <span className="text-[10px] text-amber-600 dark:text-amber-400 font-semibold">{p.reason}</span>
                  </div>
                ))}
              </div>
            </div>
